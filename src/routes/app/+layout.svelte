...
>
	<div class="p-5">
		<div class="flex items-center justify-between mb-6">
			...
			<button onclick={() => (sidebarOpen = false)} class="btn btn-ghost btn-sm btn-square" aria-label="Seitenleiste schließen">
		...
		<!-- Filter placeholder -->
		<div class="mb-6">
			<h3 class="text-xs font-semibold uppercase tracking-wider text-base-content/40 mb-3">Filter</h3>
			<p class="text-sm text-base-content/50 italic">Kommt in Sprint 2</p>
		</div>
		...
		<div class="space-y-2">
			...
			<label class="flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer hover:bg-base-200 transition-colors">
				<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
				</svg>
				<span class="text-sm flex-1">Dark Mode</span>
				<input type="checkbox" class="toggle toggle-sm" checked={$theme === 'dark'} onchange={toggleTheme} />
			</label>
...
<header class="sticky top-0 z-30 bg-base-100/80 backdrop-blur-xl border-b border-base-300">
	<div class="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
		<div class="flex items-center gap-3">
			<button onclick={() => (sidebarOpen = !sidebarOpen)} class="btn btn-ghost btn-sm btn-square" aria-label="Seitenleiste öffnen">
				<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

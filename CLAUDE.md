...
## v2 Redesign (AKTIVER ENTWICKLUNGSPFAD)

v1 ist Maintenance-only. Alle Feature-Entwicklung passiert in v2.

| Eigenschaft | Wert |
|---|---|
| Branch | `feature/v2` |
| Route | `/v2` (NICHT `/app` — das ist v1!) |
| Hauptseite | `src/routes/v2/+page.svelte` |
| Layout/Header | `src/routes/v2/+layout.svelte` |
...
**TABU:** v1-Dateien (`routes/app/`, `components/` ohne `/v2/`, `app.css`) sind bei v2-Arbeit NICHT zu bearbeiten.

Shared mit v1: `tasks.svelte.ts`, `supabase-crud.ts`, `constants.ts` — mit Vorsicht bearbeiten.
...
### Blocker vor Deploy
1. Migration 007-009 nicht auf Supabase deployed
2. Audit-Bugs: prompt() ersetzen, changeTaskProgress Subtask-Rollback, Undo-Toast Race Condition
